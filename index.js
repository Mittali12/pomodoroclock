...
document.getElementById("s_plus").addEventListener("click", function () {
  ...
  if (s_time == 2) document.getElementById("s_minus").disabled = false;
});

document.getElementById("b_plus").addEventListener("click", function () {
  b_time++;
  b_timer.innerHTML = b_time + " min";
  if (b_time == 2) document.getElementById("b_minus").disabled = false;
});

document.getElementById("s_minus").addEventListener("click", function () {
  s_time--;
  s_timer.innerHTML = s_time + " min";
  if (s_time == 1) document.getElementById("s_minus").disabled = true;
});

document.getElementById("b_minus").addEventListener("click", function () {
  b_time--;
  b_timer.innerHTML = b_time + " min";
  if (b_time == 1) document.getElementById("b_minus").disabled = true;
});

document.getElementById("start").addEventListener("click", function () {
  if (s_min == null) s_min = s_time;
  if (b_min == null) b_min = b_time;
  ...
  if (run) session_fun(s_min, s_second);
  else break_fun(b_min, b_second);
});
...
function session_fun(min, second = 0) {
  ...
  var session_interval = setInterval(function () {
    var str;
    if (second > 9) str = min + " : " + second;
    else str = min + " : 0" + second;
    if (min < 10) str = "0" + str;
